...
static const unsigned char JPEG_ToZigZag [JPEG_DCTSIZE2]; /* Converts row-major indices to zig-zagged order. */
static const unsigned char JPEG_FromZigZag [JPEG_DCTSIZE2]; /* Converts zig-zagged indices to row-major order. */
static const JPEG_FIXED_TYPE JPEG_AANScaleFactor [JPEG_DCTSIZE2]; /* AA&N scaling factors for quantisation in fixed point. */
static const unsigned char JPEG_ComponentRange [32 * 3]; /* A limited component clamp that keeps values in the 0..31 range if incremented by 32. */
...
//static int JPEG_FrameHeader_Read (JPEG_FrameHeader *frame, const unsigned char **data, JPEG_Marker marker);
...
//static int JPEG_HuffmanTable_Read (JPEG_HuffmanTable *table, const unsigned char **data);
...
//static int JPEG_HuffmanTable_Skip (const unsigned char **data);
...
//static int JPEG_ScanHeader_Read (JPEG_ScanHeader *scan, const unsigned char **data);
...
//static int JPEG_Decoder_ReadHeaders (JPEG_Decoder *decoder, const unsigned char **data);
...
//static int JPEG_Decoder_ReadImage (JPEG_Decoder *decoder, const unsigned char **data, volatile JPEG_OUTPUT_TYPE *out, int outWidth, int outHeight);
...
//static void JPEG_IDCT (JPEG_FIXED_TYPE *zz, signed char *chunk, int chunkStride);
